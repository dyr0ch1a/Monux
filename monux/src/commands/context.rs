...
use monux_core::index::{path_key, resolve_note_path, NoteIndex,
NoteMeta};
...
pub fn resolve_note_reference(index: &NoteIndex, raw: &str) ->
anyhow::Result<NoteMeta> {
    ...
    if title_exact.len() > 1 {
        anyhow::bail!("multiple notes matched title '{}'; use note
path", query);
    }
...
path_key(&note.path).to_lowercase().contains(&q_lower)
    ...
    if contains.len() > 1 {
        anyhow::bail!("multiple notes matched '{}'; refine query or
use path", query);
    }
