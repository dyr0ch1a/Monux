...
fn run(
    ...
    let mut last_motion_key: Option<(KeyCode,
crossterm::event::KeyModifiers, std::time::Instant)> =
        None;
    ...
    while !app.should_quit {
        ...
        #[cfg(not(windows))]
        {
            ...
            if last_cursor_style != Some(cursor_style) {
                match cursor_style {
                    "bar" => execute!(terminal.backend_mut(),
SetCursorStyle::SteadyBar)?,
                    "underscore" => execute!(terminal.backend_mut(),
SetCursorStyle::SteadyUnderScore)?,
                    _ => execute!(terminal.backend_mut(),
...
fn should_throttle_motion_key(
    key: &KeyEvent,
    last_motion_key: &mut Option<(KeyCode,
crossterm::event::KeyModifiers, std::time::Instant)>,
) -> bool {
    ...
    if let Some((last_code, last_mods, last_at)) =
last_motion_key.as_ref()
        && *last_code == key.code
