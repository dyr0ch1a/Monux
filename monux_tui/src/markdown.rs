...
pub fn render_markdown_segment(segment: &str, base_style: Style) -> Vec<Span<'static>> {
    ...
    if let Some((depth, consumed)) = parse_blockquote_prefix(trimmed) {
        ...
        styled_base = styled_base.bg(Color::DarkGray).fg(Color::White);
    }
...
fn render_with_prefix(
    ...
    for (event, range) in parser {
        match event {
            Event::Start(tag) => {
                let current = *style_stack.last().unwrap_or(&styled_base);
                let next = match tag {
                    Tag::Emphasis => current.add_modifier(Modifier::ITALIC),
                    Tag::Strong => current.fg(Color::White).add_modifier(Modifier::BOLD),
                    ...
                    Tag::Heading { level, .. } => heading_style_for_level(current, level),
                    _ => current,
                };
            ...
            Event::Text(_) | Event::Html(_) | Event::InlineHtml(_) => {
                let style = *style_stack.last().unwrap_or(&styled_base);
                apply_style_for_byte_range(
                    &mut styles,
                    &byte_to_char,
                    range.start,
            ...
            Event::TaskListMarker(checked) => {
                let mut style = style_stack
                    ...
                    .copied()
                    .unwrap_or(styled_base)
                    .fg(if checked { Color::Green } else { Color::Yellow });
                if checked {
    ...
    highlight_list_prefix(text, &byte_to_char, &mut styles, styled_base);

    highlight_wikilinks(text, &byte_to_char, &mut styles, styled_base);
...
fn highlight_markers(text: &str, byte_to_char: &[usize], styles: &mut [Style], base: Style) {
    ...
    while i < text.len() {
        ...
        let len = if rest.starts_with("```") {
            3
        } else if rest.starts_with("**") || rest.starts_with("__") || rest.starts_with("~~") {
            2
...
fn highlight_wikilinks(text: &str, byte_to_char: &[usize], styles: &mut [Style], base: Style) {
    let link_style = base.fg(Color::Green).add_modifier(Modifier::UNDERLINED);
    let mut i = 0usize;
    while i < text.len() {
        if !text.is_char_boundary(i) {
            i = next_char_boundary(text, i);
        ...
        {
            let total = end + 4;
            apply_style_for_byte_range(styles, byte_to_char, i, i + total, link_style);
            i += total;
            continue;
        }
        i = next_char_boundary(text, i);
...
fn highlight_list_prefix(text: &str, byte_to_char: &[usize], styles: &mut [Style], base: Style) {
    ...
    if ws_end > 0 {
        ...
        if nested_level > 0 {
            ...
            apply_style_for_byte_range(styles, byte_to_char, 0, ws_end, indent_style);
        }
    }
    ...
    if let Some(first) = rest.chars().next() {
        if (first == '-' || first == '*' || first == '+')
            && has_list_separator(rest, first.len_utf8())
        {
    ...
    if marker_len > 0 {
        ...
        let marker_style = base.fg(Color::Cyan).add_modifier(Modifier::BOLD);
        apply_style_for_byte_range(styles, byte_to_char, ws_end, marker_end, marker_style);
...
fn spans_from_chars_with_styles(chars: &[char], styles: &[Style]) -> Vec<Span<'static>> {
    if chars.is_empty() {
...
fn heading_style_for_level(base: Style, level: HeadingLevel) -> Style {
    match level {
        HeadingLevel::H1 => base.add_modifier(Modifier::BOLD).fg(Color::Cyan),
        HeadingLevel::H2 => base.add_modifier(Modifier::BOLD),
        HeadingLevel::H3 => base.add_modifier(Modifier::UNDERLINED),
        HeadingLevel::H4 | HeadingLevel::H5 | HeadingLevel::H6 => base.add_modifier(Modifier::DIM),
    }
}
...
mod tests {
    ...
    fn strong_text_gets_a_brighter_style() {
        let spans =
            render_markdown_segment("plain **bold** text", Style::default().fg(Color::DarkGray));
        let bold_span = spans
            .iter()
