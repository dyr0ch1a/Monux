...
fn draw_help_popup(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let lines = vec![
        ...
        Line::styled(
            "j/k или стрелки: прокрутка, PgUp/PgDn: страница, Home: вверх, Esc/?: закрыть",
            Style::default().add_modifier(Modifier::DIM),
        ),
        Line::from(""),
        Line::styled("Общие", Style::default().add_modifier(Modifier::BOLD)),
        Line::from("  ?: открыть или закрыть эту справку"),
        Line::from("  Tab / Shift+Tab: переключать панели"),
        Line::from("  Ctrl+N: создать заметку"),
        Line::from("  Ctrl+F: открыть глобальный поиск"),
        Line::from("  Ctrl+D: создать папку"),
        Line::from("  Ctrl+R: переименовать выбранный элемент или текущую заметку"),
        Line::from("  Ctrl+S: сохранить текущую заметку"),
        Line::from(""),
        Line::styled("Панель заметок", Style::default().add_modifier(Modifier::BOLD)),
        Line::from("  j/k или стрелки: перемещение по списку"),
        Line::from("  Enter: открыть заметку или войти в папку"),
        Line::from("  h/l или стрелки: свернуть или раскрыть папку"),
        Line::from("  /: поиск по дереву заметок"),
        Line::from("  v: режим визуального выделения"),
        Line::from("  x: вырезать выбранные заметки, p: вставить или переместить"),
        Line::from(
            "  D: удалить заметку или выбранную папку с подтверждением",
        ),
        Line::from(""),
        Line::styled(
        ...
        Line::from("  h/j/k/l или стрелки: перемещение курсора"),
        Line::from("  3j, 5k, 2h...: числовой префикс перед движением"),
        Line::from("  w/b/e: движения по словам (вперёд, назад, конец)"),
        Line::from(
            "  dd: удалить строку, dw/de/db: удалить по движению слова",
        ),
        Line::from("  i/a/o: перейти в режим вставки"),
        Line::from("  v: визуальный режим (выделение текста)"),
        Line::from("  z: свернуть или раскрыть текущий заголовок markdown"),
        Line::from(""),
        Line::styled(
            "Командный режим (:)",
            Style::default().add_modifier(Modifier::BOLD),
        ),
        Line::from("  :help                   открыть эту справку"),
        Line::from("  :w                      сохранить текущую заметку"),
        Line::from("  :q / :q!                выйти / выйти принудительно"),
        Line::from("  :mkdir <path>           создать папку"),
        Line::from(
            "  :sync [dir]             синхронизировать файлы и индекс заметок",
        ),
        Line::from("  :del <query>            удалить заметку по запросу"),
    ];
