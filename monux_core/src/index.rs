...
const TAGS_BY_PATH: TableDefinition<&str, &str> =
TableDefinition::new("tags_by_path");
...
impl NoteIndex {
    pub fn open(db_path: PathBuf, notes_root: PathBuf) ->
anyhow::Result<Self> {
    ...
    pub fn create_note(&self, name: &str) -> anyhow::Result<NoteMeta>
{
    ...
    pub fn create_note_with_tags(&self, name: &str, tags: &[String])
...
std::fs::read_to_string(&abs).unwrap_or_default();
    ...
    pub fn list(&self) -> anyhow::Result<Vec<NoteMeta>> {
        ...
        for entry in WalkDir::new(&self.notes_root) {
            let entry = entry?;
            let abs = entry.path();
            if !abs.is_file() || abs.extension().and_then(|e|
e.to_str()) != Some("md") {
                continue;
            }
    ...
    pub fn find(&self, query: &str) -> anyhow::Result<Vec<NoteMeta>> {
        ...
        let mut scored = self
            ...
            .into_iter()
            .filter_map(|note| {
                let title_score = similarity_score(&query,
&note.title);
                let path_score = similarity_score(&query,
&note.display_path());
    ...
    pub fn find_by_tag(&self, tag: &str) ->
anyhow::Result<Vec<NoteMeta>> {
...
Ok(Vec::new()),
    ...
    pub fn add_tags(&self, path: impl AsRef<Path>, tags: &[String]) ->
anyhow::Result<Vec<String>> {
    ...
    pub fn set_tags(&self, path: impl AsRef<Path>, tags: &[String]) ->
anyhow::Result<Vec<String>> {
        let rel = self.require_note_path(path.as_ref())?;
    ...
    pub fn list_tags(&self, path: impl AsRef<Path>) ->
anyhow::Result<Vec<String>> {
        let rel = self.require_note_path(path.as_ref())?;
        let abs = self.notes_root.join(&rel);
        let content =
std::fs::read_to_string(abs).unwrap_or_default();
    ...
    pub fn get_path(&self, rel: &Path) ->
anyhow::Result<Option<NoteMeta>> {
        let rel = rel.to_path_buf();
        if rel.as_os_str().is_empty() {
            return Ok(None);
        }
    ...
    pub fn delete(&self, path: impl AsRef<Path>) -> anyhow::Result<()>
{
    ...
    pub fn delete_note(&self, path: impl AsRef<Path>) ->
anyhow::Result<()> {
        let rel = resolve_note_path_ref(path.as_ref());
        if rel.as_os_str().is_empty() {
            return Ok(());
...
{}
    ...
    pub fn reindex_note(&self, path: impl AsRef<Path>) ->
anyhow::Result<()> {
        let rel = resolve_note_path_ref(path.as_ref());
        if rel.as_os_str().is_empty() {
            return Ok(());
    ...
    pub fn rename(&self, old: &Path, new: &Path) -> anyhow::Result<()>
{
    ...
    pub fn rename_note(&self, old: impl AsRef<Path>, new_name: &str)
-> anyhow::Result<NoteMeta> {
    ...
    pub fn move_note(&self, old: impl AsRef<Path>, new: impl
AsRef<Path>) -> anyhow::Result<()> {
    ...
    pub fn move_path(&self, old: &Path, new: &Path) ->
anyhow::Result<()> {
        ...
        if self.get_path(&new_rel)?.is_some() {
            anyhow::bail!("note '{}' already exists",
path_key(&new_rel));
        }
    ...
    pub fn rename_dir(&self, old_raw: &str, new_raw: &str) ->
anyhow::Result<usize> {
        self.move_dir(old_raw, new_raw)
    }


    pub fn move_dir(&self, old_raw: &str, new_raw: &str) ->
anyhow::Result<usize> {
        ...
        if new_dir.starts_with(&(old_dir.clone() + "/")) {
            anyhow::bail!("cannot rename directory into its own
child");
        }
    ...
    pub fn delete_dir(&self, raw_dir: &str) -> anyhow::Result<()> {
        ...
        let has_notes_inside = self
            ...
            .iter()
            .any(|note|
path_key(&note.path).starts_with(&dir_prefix));
        if has_notes_inside {
...
{}
    ...
    fn require_note_path(&self, path: impl AsRef<Path>) ->
    ...
    fn move_tags(&self, old: &Path, new: &Path) -> anyhow::Result<()>
{
        ...
        {
            let mut table = write_txn.open_table(TAGS_BY_PATH)?;
            let old_tags = table.get(old_key.as_str())?.map(|v|
v.value().to_string());
    ...
    fn rewrite_prefix(&self, old_prefix: &Path, new_prefix: &Path) ->
anyhow::Result<()> {
...
self.rewrite_prefix_recursive_with_keys(&old_prefix_key, &new_prefix_key)?;
    ...
    fn rewrite_prefix_recursive(&self, old_prefix: &Path, new_prefix:
&Path) -> anyhow::Result<usize> {
        let old_prefix_key = format!("{}/", path_key(old_prefix));
        let new_prefix_key = format!("{}/", path_key(new_prefix));
        self.rewrite_prefix_recursive_with_keys(&old_prefix_key,
&new_prefix_key)
    }
        ...
        let moved;
        {
            ...
            for entry in table.iter()? {
                ...
                if let Some(rest) = key.strip_prefix(old_prefix_key) {
                    ...
                    replacements.push((key.to_string(), new_key,
value_guard.value().to_string()));
    ...
    fn store_tags(&self, rel: &Path, tags: &[String]) ->
anyhow::Result<()> {
        let abs = self.notes_root.join(rel);
        let current =
std::fs::read_to_string(&abs).unwrap_or_default();
    ...
    fn note_meta_for_rel(
        ...
        let title = if abs.is_file() {
            let content =
std::fs::read_to_string(&abs).unwrap_or_default();
            note_title_from_content(&content).unwrap_or_else(||
title_from_path(&rel))
        } else {
            fallback_name
                .filter(|s| !s.is_empty())
...
fn levenshtein(a: &str, b: &str) -> usize {
    ...
    for (i, a_ch) in a.chars().enumerate() {
        ...
        for (j, b_ch) in b_chars.iter().enumerate() {
            ...
            curr[j + 1] = (curr[j] + 1).min(prev[j + 1] +
1).min(prev[j] + cost);
        }
...
fn normalize_tags(tags: &[String]) -> Vec<String> {
    ...
    for tag in tags {
        ...
        if !normalized.is_empty() && !out.iter().any(|existing|
existing == &normalized) {
            out.push(normalized);
        }
    }
...
fn write_tags_to_markdown(content: &str, tags: &[String]) -> String {
    ...
    let mut lines = existing_frontmatter
        .map(|fm|
fm.lines().map(ToString::to_string).collect::<Vec<String>>())
        .unwrap_or_default()
        .into_iter()
...
fn parse_tags_frontmatter_value(raw: &str) -> Vec<String> {
    ...
    let tokens = inner
        ...
        .map(|tag|
tag.trim().trim_matches('"').trim_matches('\'').to_string())
        .collect::<Vec<_>>();
    normalize_tags(&tokens)
}
