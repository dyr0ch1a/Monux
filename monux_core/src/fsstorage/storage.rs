...
e.to_str()) != Some("md") {
    ...
    pub fn create_note(&self, rel: &Path, content: &str) ->
anyhow::Result<()> {
    ...
    pub fn rename_note(&self, old_rel: &Path, new_rel: &Path) ->
anyhow::Result<()> {
        let old_abs = self.notes_root.join(old_rel);
        let new_abs = self.notes_root.join(new_rel);
        if !old_abs.exists() {
            anyhow::bail!("note '{}' not found on disk",
path_key(old_rel));
        }
...
std::fs::read_to_string(abs).unwrap_or_default();
    ...
    pub fn write_tags(&self, rel: &Path, tags: &[String]) ->
